...
// Edit these values for your MySQL server.
$host = '10.1.1.100';
$username = 'bryce';
$password = '1234';
$database = 'final';
        ...
        switch ($action) {
            case 'add_user':
                run(
                    ...
                    . sql($conn, trim($_POST['username'] ?? '')) . ', '
                    . sql($conn, trim($_POST['hashed_password'] ?? '')) . ', '
                    . sql($conn, (float) ($_POST['cash_balance'] ?? 0)) . ')'
                );
                $messages[] = 'User added.';
                break;
            ...
            case 'add_market':
                ...
                run(
                    ...
                    . sql($conn, trim($_POST['event_description'] ?? '')) . ', '
                    . sql($conn, trim($_POST['status'] ?? '')) . ', '
                    . sql($conn, $resolveDate) . ', '
                    . sql($conn, trim($_POST['outcome'] ?? '')) . ')'
                );
            ...
            case 'add_offer':
                run(
                    ...
                    . sql($conn, trim($_POST['username'] ?? '')) . ', '
                    . sql($conn, (int) ($_POST['market_id'] ?? 0)) . ', '
                    . sql($conn, trim($_POST['contract_type'] ?? '')) . ', '
                    . sql($conn, trim($_POST['side'] ?? '')) . ', '
                    . sql($conn, (float) ($_POST['price_per_share'] ?? 0)) . ', '
                    . sql($conn, (int) ($_POST['quantity'] ?? 0)) . ')'
                );
            ...
            case 'delete_row':
                ...
                if ($table === 'users') {
                    run($conn, 'DELETE FROM users WHERE username = ' . sql($conn, $id));
                } elseif ($table === 'markets') {
                    run($conn, 'DELETE FROM markets WHERE market_id = ' . sql($conn, (int) $id));
                } elseif ($table === 'offers') {
                    run($conn, 'DELETE FROM offers WHERE offer_id = ' . sql($conn, (int) $id));
                } elseif ($table === 'transactions') {
                    run($conn, 'DELETE FROM transactions WHERE transaction_id = ' . sql($conn, (int) $id));
                } else {
...
if ($q1Status !== '') {
    $q1Sql .= ' AND status = ' . sql($conn, $q1Status);
}
if ($q1Keyword !== '') {
    $q1Sql .= ' AND event_description LIKE ' . sql($conn, '%' . $q1Keyword . '%');
}
...
$q2Sql = "SELECT o.offer_id, o.username, m.event_description, o.contract_type, o.side, o.price_per_share, o.quantity, o.created_at
          ...
          WHERE 1 = 1";
if ($q2User !== '') {
    $q2Sql .= ' AND o.username = ' . sql($conn, $q2User);
}
if ($q2Side !== '') {
    $q2Sql .= ' AND o.side = ' . sql($conn, $q2Side);
}
...
$q3Sql = "SELECT t.transaction_id, t.username, m.event_description, t.contract_type, t.side, t.price, t.quantity, t.transacted_at
          ...
          WHERE 1 = 1";
if ($q3User !== '') {
    $q3Sql .= ' AND t.username = ' . sql($conn, $q3User);
}
if ($q3MarketId !== '' && ctype_digit($q3MarketId)) {
    $q3Sql .= ' AND t.market_id = ' . sql($conn, (int) $q3MarketId);
}
...
$q4Sql = "SELECT m.market_id,
          ...
          WHERE 1 = 1";
if ($q4Status !== '') {
    $q4Sql .= ' AND m.status = ' . sql($conn, $q4Status);
}
$q4Sql .= ' GROUP BY m.market_id, m.event_description, m.status';
if ($q4MinOffers !== '' && ctype_digit($q4MinOffers)) {
    $q4Sql .= ' HAVING COUNT(DISTINCT o.offer_id) >= ' . sql($conn, (int) $q4MinOffers);
}
...
<body>
    <div class="page">
        <div class="hero">
            ...
            <div class="stats">
                <div class="stat"><strong>Users</strong><br><?= h(count($users)) ?></div>
                <div class="stat"><strong>Markets</strong><br><?= h(count($markets)) ?></div>
                <div class="stat"><strong>Offers</strong><br><?= h(count($offers)) ?></div>
                <div class="stat"><strong>Transactions</strong><br><?= h(count($transactions)) ?></div>
            </div>
        </div>
                ...
                <form method="post">
                    ...
                    <select name="market_id" required>
                        ...
                        <?php foreach ($marketOptions as $marketId): ?>
                            <option value="<?= h($marketId) ?>"><?= h($marketId) ?></option>
                        <?php endforeach; ?>
                    </select>
        ...
        <div class="query-grid">
            <div class="card">
                ...
                <form method="get">
                    ...
                    <input name="q1_keyword" value="<?= h($q1Keyword) ?>" placeholder="keyword in event description">
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($query1Rows as $row): ?>
                            <tr>
                                <td><?= h($row['market_id']) ?></td>
                                <td><?= h($row['event_description']) ?></td>
                                <td><?= h($row['status']) ?></td>
                                <td><?= h($row['resolve_date']) ?></td>
                                <td><?= h($row['outcome']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="card">
                ...
                <form method="get">
                    <select name="q2_user">
                        ...
                        <?php foreach ($userOptions as $user): ?>
                            <option value="<?= h($user) ?>" <?= $q2User === $user ? 'selected' : '' ?>><?= h($user) ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select name="q2_side">
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($query2Rows as $row): ?>
                            <tr>
                                <td><?= h($row['offer_id']) ?></td>
                                <td><?= h($row['username']) ?></td>
                                <td><?= h($row['event_description']) ?></td>
                                <td><?= h($row['contract_type']) ?></td>
                                <td><?= h($row['side']) ?></td>
                                <td>$<?= h(number_format((float) $row['price_per_share'], 2)) ?></td>
            ...
            <div class="card">
                ...
                <form method="get">
                    <select name="q3_user">
                        <option value="">any user</option>
                        <?php foreach ($userOptions as $user): ?>
                            <option value="<?= h($user) ?>" <?= $q3User === $user ? 'selected' : '' ?>><?= h($user) ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select name="q3_market_id">
                        <option value="">any market</option>
                        <?php foreach ($marketOptions as $marketId): ?>
                            <option value="<?= h($marketId) ?>" <?= $q3MarketId === (string) $marketId ? 'selected' : '' ?>><?= h($marketId) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($query3Rows as $row): ?>
                            <tr>
                                <td><?= h($row['transaction_id']) ?></td>
                                <td><?= h($row['username']) ?></td>
                                <td><?= h($row['event_description']) ?></td>
                                <td><?= h($row['contract_type']) ?></td>
                                <td><?= h($row['side']) ?></td>
                                <td>$<?= h(number_format((float) $row['price'], 2)) ?></td>
                                <td><?= h($row['quantity']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="card">
                ...
                <form method="get">
                    ...
                    <input name="q4_min_offers" type="number" min="0" value="<?= h($q4MinOffers) ?>" placeholder="minimum open offers">
                    ...
                    <tbody>
                        <?php foreach ($query4Rows as $row): ?>
                            <tr>
                                ...
                                <td><?= h($row['event_description']) ?></td>
                                <td><?= h($row['status']) ?></td>
                                <td><?= h($row['open_offers']) ?></td>
                                <td><?= h($row['total_transactions']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="card">
                ...
                <form method="get">
                    <input name="q5_min_cash" type="number" step="0.01" min="0" value="<?= h($q5MinCash) ?>" placeholder="minimum cash balance">
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($query5Rows as $row): ?>
                            <tr>
                                <td><?= h($row['username']) ?></td>
                                <td>$<?= h(number_format((float) $row['cash_balance'], 2)) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($users as $row): ?>
                        <tr>
                            <td><?= h($row['username']) ?></td>
                            <td><?= h($row['hashed_password']) ?></td>
                            <td>$<?= h(number_format((float) $row['cash_balance'], 2)) ?></td>
                            <td>
                                <form method="post" class="inline-form">
                                    <input type="hidden" name="action" value="delete_row">
                                    <input type="hidden" name="table" value="users">
                                    <input type="hidden" name="id" value="<?= h($row['username']) ?>">
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" class="inline-form">
                                    ...
                                    <input type="hidden" name="id" value="<?= h($row['market_id']) ?>">
                                    <button class="delete" type="submit">Delete</button>
                                </form>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" class="inline-form">
                                    ...
                                    <input type="hidden" name="id" value="<?= h($row['offer_id']) ?>">
                                    <button class="delete" type="submit">Delete</button>
                                </form>
                            </td>
                        </tr>
                ...
                <tbody>
                    <?php foreach ($transactions as $row): ?>
                        <tr>
                            ...
                            <td>$<?= h(number_format((float) $row['price'], 2)) ?></td>
                            <td><?= h($row['quantity']) ?></td>
                            <td><?= h($row['transacted_at']) ?></td>
                            <td>
                                <form method="post" class="inline-form">
                                    <input type="hidden" name="action" value="delete_row">
                                    <input type="hidden" name="table" value="transactions">
                                    <input type="hidden" name="id" value="<?= h($row['transaction_id']) ?>">
                                    <button class="delete" type="submit">Delete</button>
                                </form>
                            </td>
                        </tr>
